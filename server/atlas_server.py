...
#
# Lives in atlas-core/server/ because every endpoint here (popstats / LD /
# dosage / file IO / compute) is a generic data-server concern. The atlas
# packages on top (inversion, future diversity, future genome) all share
# this same server; per-atlas specialization lives in registry JSONs and
# operations registries, NOT in server code.
#
# This file owns BOTH halves of the previous split-server setup: the
# popstats / LD / dosage / ancestry endpoints (the original job) AND the
# lightweight /file + /compute endpoints. The two subsystems boot
# independently:
#
...
#
# Migration note:
#   This file was previously named popstats_server.py and lived under
#   inversion-atlas/atlases/inversion/server/. It moved to
#   atlas-core/server/atlas_server.py because every endpoint here is
#   generic plumbing — popstats / LD / dosage / file IO / compute — and
#   nothing about it is inversion-specific.
# =============================================================================
...
#   1. `python3 atlas_server.py --config <path>` — main() calls _bootstrap
#   2. `uvicorn atlas_server:app` — startup hook reads POPSTATS_CONFIG env
...
# turn 146 — static-files subsystem state. When set, the server also serves
# the assembled atlas workspace (index.html + core/ + atlases/ + …) from /,
# so the browser hits ONE port for both static files AND popstats / file /
# compute APIs. Set by _bootstrap_static at startup. Optional — when WORKSPACE_ROOT
...
async def _lifespan(app: FastAPI):
    ...
    # Honor POPSTATS_CONFIG=<path> if set so `uvicorn atlas_server:app`
    # works the same as `python atlas_server.py --config <path>`.
    if ENGINES is None:
    ...
    # turn 146 — same pattern for static-files subsystem. Honors
    # ATLAS_WORKSPACE_ROOT so `uvicorn atlas_server:app --reload`
    # serves the workspace identically to the CLI path.
    if WORKSPACE_ROOT is None:
        ws_path = os.environ.get("ATLAS_WORKSPACE_ROOT")
        if ws_path:
            try:
...
# =============================================================================
# Write-path allowlist (registry v2 contract)
# =============================================================================
# Registry.write() goes through POST /file/{path}. The atlas-side Registry
# already enforces writable: true on the layer entry, but we don't trust
# the client — this is the second line of defence.
#
# Allowed write prefixes (matches DATA_LIFECYCLE.md and the registry v2 doc):
#
#   1. data/candidates/{cid}/lineage.json
#        — the version index for a candidate
#   2. data/candidates/{cid}/{version_id}/<aspect>.json
#        — per-version aspects (boundaries, gene_cargo, sv_counts, marker_primers,
#          breeding_readiness_card, final_classification, etc.)
#   3. data/arrangement_calls/{cid}/{version_id}/<file>
#        — versioned arrangement calls
#   4. data/review/{atlas_id}/sessions/{session_id}/<file>
#        — review session writes (any atlas; atlas_id matched as a single
#          path segment so cross-atlas writes can't escape via "..")
#   5. _cache/server_results/{op_id}/{hash}.json
#        — server compute persist hook (registry v2 persist: true layers)
#
# Anything else: 403. Adding a new write surface requires editing this
# function explicitly, on purpose.
#
# Path 5 is special: when the request path starts with _cache/server_results/,
# the file is rewritten to live under SERVER_RESULTS_CACHE_ROOT (configured
# via atlas_server.config.yaml > server_results_cache_root, default
# /mnt/e/inversion-atlas-cache/server_results/). This keeps test-run cache
# bloat off the project tree.
# =============================================================================

# Default cache root if config doesn't set one. Configurable via YAML.
_DEFAULT_SERVER_RESULTS_CACHE_ROOT = "/mnt/e/atlas-cache/server_results"
SERVER_RESULTS_CACHE_ROOT: Optional[Path] = None


def _is_path_allowed_for_write(rel: str) -> bool:
    """Return True iff `rel` (a path relative to PROJECT_ROOT, as it
    arrives in POST /file/{path}) is on the registry-v2 write allowlist.

    Matches the five prefixes documented above. The matching is on the
    REQUEST string, not on a resolved Path — _safe_project_path
    independently guards against traversal. Both checks run.
    """
    # Normalise: strip leading slash, collapse any leading "./".
    p = rel.lstrip('/').lstrip('./').lstrip('/')

    # Path 5 — server results cache (most common write target).
    if p.startswith('_cache/server_results/'):
        # Require at least op_id/hash.json shape: 3 segments after prefix.
        rest = p[len('_cache/server_results/'):]
        if rest and '/' in rest and rest.endswith('.json'):
            return True
        return False

    # Path 4 — review sessions for any atlas:
    # data/review/<atlas_id>/sessions/<session_id>/<file>
    # We require {atlas_id} and {session_id} to be single path segments
    # (no slashes, no "..") so the allowlist doesn't widen by accident.
    if p.startswith('data/review/'):
        parts = p.split('/')
        # data review <atlas_id> sessions <session_id> <...rest>
        if (len(parts) >= 6
    ...
    # Path 3 — versioned arrangement calls.
    if p.startswith('data/arrangement_calls/'):
        # Require {cid}/{version_id}/<file>: at least 4 path segments.
        parts = p.split('/')
        if len(parts) >= 4:
            return True
        return False

    # Paths 1+2 — per-candidate writes.
    if p.startswith('data/candidates/'):
        parts = p.split('/')
        # data/candidates/{cid}/lineage.json -> 4 segments, last == "lineage.json"
        if len(parts) == 4 and parts[3] == 'lineage.json':
            return True
        # data/candidates/{cid}/{version_id}/<aspect>.json -> 5 segments
...
def _resolve_write_target(rel: str) -> Path:
    """Resolve a write-target path, redirecting the server results cache
    prefix to the configured filesystem root.

    For all other allowed prefixes: same as _safe_project_path(rel).
    For _cache/server_results/<rest>: returns SERVER_RESULTS_CACHE_ROOT/<rest>
    (creating parents as needed). Treats SERVER_RESULTS_CACHE_ROOT as a
    write-only sandbox; we don't enforce it being inside PROJECT_ROOT
    because the whole point is for it to live on a separate disk.
    """
    p = rel.lstrip('/').lstrip('./').lstrip('/')
    if p.startswith('_cache/server_results/'):
        rest = p[len('_cache/server_results/'):]
        # Light traversal guard on the rest portion.
        if '..' in rest.split('/'):
            raise HTTPException(403, f"path escapes server results cache: {rel}")
        root = SERVER_RESULTS_CACHE_ROOT or Path(_DEFAULT_SERVER_RESULTS_CACHE_ROOT)
...
def _bootstrap_static(workspace_root: Path) -> None:
    """turn 146 — bootstrap the static-files subsystem.

    Sets the global WORKSPACE_ROOT and mounts a StaticFiles handler at /
    so this single server can serve both the assembled atlas workspace
    AND the popstats / file / compute APIs. The mount goes LAST so that
    any previously-registered API route takes precedence; only paths
    that no API route claims fall through to the static handler.

    Required workspace layout (verified against the assemble.sh recipe):
    ...
    The function is forgiving: it warns (not errors) on missing pieces
    so a half-built workspace can still be served — the browser will
    surface concrete errors at boot, which is more useful for debugging
    ...
    # Sanity warnings (not errors): point out a workspace that isn't going
    # to work, but let the user proceed — they may be mid-assembly.
    if not (root / "index.html").exists():
        log.warning("workspace root has no index.html: %s "
                    "(did you mean to point at the atlas-workspace/ folder, "
                    "not its parent?)", root)
    if not (root / "atlases" / "_index.json").exists():
        log.warning("workspace root has no atlases/_index.json: %s "
    ...
    # falls through to the static mount and is served from disk.
    #
    # html=True means a request for / serves index.html automatically.
    # check_dir=False means we don't fail mount-time on a non-existent
    # path — this matches the bootstrap's "forgiving" stance.
    app.mount(
        "/",
        StaticFiles(directory=str(root), html=True, check_dir=False),
...
# Without this, browsers happily serve a 5-minute-old core/atlas_router.js
# even after you've edited it on disk. The atlas dev workflow is
# edit-on-disk + browser-refresh; a stale-cache hit silently undoes that.
# This middleware adds Cache-Control: no-store to every response that
# isn't an API call. The runtime cost is one header per response, which
# is negligible for a local dev server.
@app.middleware("http")
...
def _static_subsystem_status() -> Dict[str, Any]:
    """Build the static-files subsystem block of /health.

    Reports whether the workspace mount is live and where it points.
    Atlas frontend doesn't need this — it's strictly for human / CI
    inspection ('did the server actually mount the workspace I asked
    for?'). Cheap to compute.
    """
    if WORKSPACE_ROOT is None:
        return {
            "ready": False,
            "reason": "static subsystem not configured. Pass "
                      "--workspace-root <dir> (or ATLAS_WORKSPACE_ROOT env) to enable.",
        }
    has_index = (WORKSPACE_ROOT / "index.html").exists()
    has_atlases = (WORKSPACE_ROOT / "atlases" / "_index.json").exists()
    return {
        "ready": True,
        "workspace_root": str(WORKSPACE_ROOT),
        "has_index_html": has_index,
        "has_atlases_index_json": has_atlases,
    }


@app.get("/api/health")
async def health() -> Dict[str, Any]:
    """Unified health probe. Reports all three subsystems independently.

    Top-level `ok` is true if ANY subsystem is ready (server is doing
    ...
    static   = _static_subsystem_status()
    return {
        "ok":         popstats["ready"] or file_io["ready"] or static["ready"],
...
async def file_get(path: str) -> Response:
    ...
    p_norm = path.lstrip('/').lstrip('./').lstrip('/')
    if p_norm.startswith('_cache/server_results/'):
        rest = p_norm[len('_cache/server_results/'):]
        if '..' in rest.split('/'):
            raise HTTPException(403, f"path escapes server results cache: {path}")
        root = SERVER_RESULTS_CACHE_ROOT or Path(_DEFAULT_SERVER_RESULTS_CACHE_ROOT)
        target = (root / rest).resolve()
        try:
            target.relative_to(root.resolve())
...
async def file_post(path: str, request: Request) -> Dict[str, Any]:
    # Registry v2 contract: every write must be on the allowlist. The
    # client-side Registry already checks writable: true; this is the
    # second line of defence and it cannot be bypassed.
    if not _is_path_allowed_for_write(path):
        raise HTTPException(
            403,
            f"path not allowed for write: {path}. "
            f"Permitted prefixes: data/candidates/{{cid}}/lineage.json, "
            f"data/candidates/{{cid}}/{{version_id}}/<aspect>.json, "
            f"data/arrangement_calls/{{cid}}/{{version_id}}/<file>, "
            f"data/review/inversion/sessions/<...>, "
            f"_cache/server_results/{{op_id}}/{{hash}}.json"
        )
    # _resolve_write_target rewrites _cache/server_results/* to the
    # configured filesystem root (default /mnt/e/inversion-atlas-cache/server_results/);
    # all other allowed prefixes go to PROJECT_ROOT/<path>.
...
async def ld_split_heatmap(req: LDSplitReq) -> Response:
    ...
    try:
        ...
        raise HTTPException(
            ...
            "in atlas_server.config.yaml. Typical path: "
...
async def dosage_chunk(chrom: str, start: int, end: int,
    ...
    if not dosage_dir.is_dir():
        raise HTTPException(status_code=500, detail={
            ...
            "hint": "Set 'dosage_dir' in atlas_server.config.yaml.",
        })
...
async def dosage_manifest() -> Dict[str, Any]:
    ...
    return {
        "ok": True,
        "service": "atlas_server.dosage_bridge",
...
def _bootstrap(config_path: Path) -> None:
    global CFG, ENGINES, CACHE, SAMPLES, SERVER_RESULTS_CACHE_ROOT
    ...
    # Registry v2 — server results cache root (per persist contract).
    # Default lives outside the project tree on the user's big disk so
    # cached test runs don't bloat tarballs / git.
    cache_root = CFG.get("server_results_cache_root", _DEFAULT_SERVER_RESULTS_CACHE_ROOT)
    SERVER_RESULTS_CACHE_ROOT = Path(cache_root).expanduser().resolve()
    try:
        SERVER_RESULTS_CACHE_ROOT.mkdir(parents=True, exist_ok=True)
        log.info("server results cache root: %s", SERVER_RESULTS_CACHE_ROOT)
    except Exception as e:
        # Don't fail bootstrap if the cache root isn't writable yet — log
        # and let the first persist attempt surface a clearer error.
        log.warning("could not create server results cache root %s: %s",
                    SERVER_RESULTS_CACHE_ROOT, e)
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="atlas server (merged) — unified popstats + /file + /compute + static server")
    ap.add_argument("--config", default=None, type=Path,
                    help="path to atlas_server.config.yaml — enables "
    ...
    ap.add_argument("--workspace-root", default=None, type=Path,
                    help="path to the assembled atlas-workspace folder — enables "
                         "static file serving from / so the same port serves both "
                         "the atlas UI and the popstats / file / compute APIs. "
                         "If omitted but --project-root is set, defaults to it. "
                         "Pass an empty string to disable static-file serving.")
    ...
    # turn 146 — static subsystem: optional. Defaults to --project-root
    # when --workspace-root is not given (typical case: same folder for
    # both file IO and static serving). Pass --workspace-root='' to
    # explicitly disable (API-only mode).
    ws_root = args.workspace_root
    if ws_root is None and PROJECT_ROOT is not None:
        ws_root = PROJECT_ROOT
    if ws_root is not None and str(ws_root) != "":
        try:
            _bootstrap_static(Path(ws_root))
        except Exception as e:
            log.error("--workspace-root bootstrap failed: %s "
                      "(static file serving will be unavailable)", e)
    else:
        log.info("no --workspace-root given; static file serving disabled")
    ...
    # Sanity: refuse to start if NO subsystem is alive
    if PROJECT_ROOT is None and ENGINES is None and WORKSPACE_ROOT is None:
        log.error("no subsystem could be started — refusing to launch")
        sys.exit(2)

    log.info("listening on http://%s:%d", host, port)
    log.info("subsystems: file=%s, popstats=%s, static=%s",
             "ready" if PROJECT_ROOT is not None else "disabled",
             "ready" if ENGINES is not None else "disabled",
             "ready" if WORKSPACE_ROOT is not None else "disabled")
    if WORKSPACE_ROOT is not None:
        log.info("    open this in your browser:  http://%s:%d/", host, port)

    # uvicorn.run("atlas_server:app", ...) re-imports the module in its
    ...
    # the same state. All three env vars are read by _lifespan() above.
