...
> **Note on syntax.** References below to `reg$evidence$write_block(...)`
> reflect the LANTA-toolkit R surface. In the atlas registry, the
> equivalent is `await registry.write('candidate_<aspect>',
> { candidate_id, version_id }, payload)` where `<aspect>` matches the
> structured-block name (e.g. `boundary_refined`, `recombinant_map`,
> `internal_dynamics`). The data shapes, key definitions, and
> downstream-consumer descriptions in this doc are canonical — only
