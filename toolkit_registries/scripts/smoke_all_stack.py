...
def main():
    ...
    steps = [
        ...
        _run("check_chunks", [sys.executable, "scripts/check_chunks.py"], rel,
             check_stdout=["OK"]),
        _run("check_panels", [sys.executable, "scripts/check_panels.py"], rel,
             check_stdout=["OK"]),
        _run("check_external_databases",
             [sys.executable, "scripts/check_external_databases.py"], rel,
             check_stdout=["OK"]),
        _run("check_addons",
             [sys.executable, "scripts/check_addons.py"], rel,
             check_stdout=["OK"]),
        _run("check_derived_objects",
             [sys.executable, "scripts/check_derived_objects.py"], rel,
             check_stdout=["OK"]),
        _run("derived_object_harvester (candidate --all)",
             [sys.executable, "-m", "lib.derived_object_harvester",
              "--kind", "candidate", "--all"], rel,
             check_stdout=["2 candidate(s) harvested"]),
        _run("derived_object_harvester (LRR_regime --all)",
             [sys.executable, "-m", "lib.derived_object_harvester",
              "--kind", "LRR_regime", "--all"], rel,
             check_stdout=["3 LRR_regime(s) harvested"]),
        _run("refresh_references (cassette mode)",
             [sys.executable, "lib/refresh_references.py",
              "--fixture", "lib/refresh_references_fixture.example.json"], rel,
             check_stdout=["changed: 2 / scanned: 18"]),
        _run("build_panel_coverage_graph",
             [sys.executable, "-m", "lib.build_panel_coverage_graph"], rel,
             check_stdout=["wrote"]),
        _run("build_layer_graph",
             [sys.executable, "-m", "lib.build_layer_graph"], rel,
             check_stdout=["wrote"]),
        _run("build_chain_graph",
             [sys.executable, "-m", "lib.build_chain_graph"], rel,
             check_stdout=["wrote"]),
        _run("build_atlas_dependency_graph",
             [sys.executable, "-m", "lib.build_atlas_dependency_graph"], rel,
             check_stdout=["wrote"]),
